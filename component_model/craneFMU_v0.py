from pythonfmu import Fmi2Causality, Fmi2Initial, Fmi2Variability, Fmi2Slave, Boolean, Integer, Real, String, DefaultExperiment # for work with PythonFMU

class CraneFMU(Fmi2Slave):
    '''Fmi2Slave implementation of the crane model, which can be compiled into an FMU through buildFMU

    Args:
       crane (obj): reference to the (instantiated) crane object
       author (str)=me: The author of the FMU
       version (str)=0.1: The version number of the present FMU
    ...
    '''
    def __init__(self, crane:obj,
                 author:str = 'Siegfried Eisinger, DNV', version='0.1',
                 licenseTxt:str='Open Source', copyrightTxt:str='See copyright notice of used tools',
                 defaultExperiment:dict=None, non_default_flags:dict=Noneguid=None, **kwargs):
        self.crane = crane
        if defaultExperiment is None: self.default_experiment = DefaultExperiment( None, None, None, None) else DefaultExperiment( **defaultExperiment)
        kwargs.update( { 'instance_name':__name__, 'modelName':self.crane.name, 'description':self.crane.description,
                         'author':author, 'version':version, 'license':licenseTxt, 'copyright':copyrightTxt,
                         'guid':guid if guid is not None else uuid.uuid4().hex, 
                         'default_experiment':self.default_experiment})
        super().__init__( **kwargs)
        self.non_default_flags = check_flags( non_default_flags)
...
#         self.register_variable(String("mdShort", causality=Fmi2Causality.local))
    ...
    def check_flags( flags):
        def check_flag( fl, typ):
            if fl in flags:
                if isinstance( flags[fl], bool) and flags[fl]: # a non-default value
                    _flags.update( { fl, flags[fl]})
                elif isinstance( flags[fl], int) and flags[fl]!=0: # not-default integer
                    _flags.update( { fl, flags[fl]})
        _flags = {}
    ...
    def to_xml(self, el):
        ...
        if el.tag == 'CoSimulation': #ToDo: may need to check whether also other information than the flags need to be included
            el.attrib.update( self.none_default_flags)