...
class Test_pint(unittest.TestCase):
    ...
    def test_needed_functions(self):
        ...
        print(
            ...
            "degrees_Celsius" in uReg,
        )
        print("Unit System", uReg.default_system)
        print("Implicit and explicit def", 0.2 * uReg.kg, uReg.Quantity(0.2, uReg.kg))
        print(
            "String parsing:", uReg("0.2kg"), uReg(" 0.2 kg"), uReg("kg"), uReg("1.3e5")
        )
        print(
            "Base Units",
            uReg.Quantity(1.0, "ft").to_base_units(),
            uReg("0.2 kg").to_base_units(),
        )
        print(
            "Disect",
            uReg.Quantity(1.0, "ft").magnitude,
            uReg.Quantity(1.0, "ft").units,
            uReg.Quantity(1.0, "ft").dimensionality,
        )
        print(
            "Temperature",
            uReg.Quantity(38.0, uReg.degC),
            uReg("38.0*degK"),
            uReg.Quantity(38.0, "degF"),
            uReg.Quantity(38.0, "degF").to_base_units(),
        ...
        print(
            "dimensionality of N: ",
            "".join(
                x + ":" + str(q.dimensionality[x]) + ", " for x in q.dimensionality
            ),
        )
        ...
        self.assertTrue(
            q.is_compatible_with("square_mile")
        )  # check compatibility with a given unit
        self.assertTrue(
            q.check("[area]")
        )  # check compatibility with derived dimensions
        q = uReg("1 mol")
        ...
        print(
            ...
            q.to_base_units(),
            q.dimensionality,
            uReg.get_base_units(list(uReg.get_compatible_units(q.dimensionality))[0]),
        )
        q = uReg("9 degrees/s")
        print(
            "RAD/s:",
        ...
        print(
            "30 degrees in base units: ",
            uReg("30 deg").to_base_units().magnitude + " radians.",
        )
    ...
    def test_split(self):
        ...
        self.assertEqual(
            tuple([Unit.split(u) for u in ("3m", "45 deg", 0)]),
            ((3, "m"), (45, "deg"), (0, "")),
        )
    ...
    def test_quantity(self):
        ...
        self.assertEqual(
            Unit.quantity("-10 deg"), (-0.17453292519943295, "deg", "angle")
        )
