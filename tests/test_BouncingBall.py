...
class BouncingBallFMU(Model):
    ...
    def enter_initialization_mode(self):
        self.energy.value = 9.81 * self.x.value[1] + 0.5 * np.dot(
            self.v.value, self.v.value
        )
        self.period.value = (
            2 * self.v.value[1] / 9.81
        )  # may change when energy is taken out of the system
        return True
    ...
    def do_step(self, current_time, step_size):
        ...
        if y <= 0 and self.v.value[1] < 0:  # bounce
            t0 = (
                self.v.value[1]
                / 9.81
                * (1 - sqrt(1 + 2 * self.x.value[1] * 9.81 / self.v.value[1] ** 2))
            ...
            while True:
                ...
                ):  # cannot do a whole bounce in the remaining time
                    break
                if self.drag.value != 0:
        ...
        if self.drag != 0:
            ...
            self.v.value *= fac
            self.energy.value = 9.81 * self.x.value[0] + 0.5 * np.dot(
                self.v.value, self.v.value
            )
