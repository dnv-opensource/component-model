...
class Get_from_code( SphinxDirective):
    ...
    def run(self):
        ...
        if len( self.arguments) == 1: self.typ = None
        else:                         self.typ = self.arguments[1]
        ...
        print("INFO [" +self.arguments[0] +"] Type:", type(obj), ", __name__: "+type(obj).__name__, ", type: ", self.typ)
        if self.typ is not None:
            if self.typ == 'code':
                text = obj.__code__ #TODO: this is only the code object. Need to retrieve the code text and format it appropriately
            else:
...
def setup(app):
    app.add_directive("get-from-code", Get_from_code)
    return( {
        'version': '0.1',
